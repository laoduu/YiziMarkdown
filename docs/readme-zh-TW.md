...
- **匯出**：HTML / Markdown / 純文字 / Word（DOCX）/ PDF 五種格式；DOCX 使用 Word 原生語意（清單/表格/程式碼區塊/引用/任務清單），表格樣式對齊預覽，圖片支援本機路徑、網路圖片與 HTML `<img>` 標籤（可用 `width` 屬性控制寬度）；PDF 與預覽還原度一致並自動分頁
- **.md 文件關聯**：設定中一鍵設為系統預設 Markdown 編輯器，雙擊 .md 直接打開（Windows 登錄檔 / macOS LaunchServices）

### 雲端儲存（WebDAV）

- **雲端檔案瀏覽器**：側邊欄「雲端」分頁瀏覽 WebDAV 伺服器目錄樹，支援開啟文件、新增資料夾、重新命名、刪除
- **儲存到雲端**：本機文件一鍵上傳（文件中以相對路徑引用的圖片會一併上傳並保持目錄結構）；已是雲端文件則 `Ctrl+S` 直接寫回伺服器
- **衝突保護**：儲存前比對伺服器 ETag，遠端被其他裝置改過會彈窗讓你選擇「覆寫伺服器版本」或「放棄本機修改並重新載入」，絕不靜默覆寫
- **認證安全**：使用者名稱與密碼存入系統憑證儲存庫（Windows 認證管理員 / macOS 鑰匙圈），不寫入任何設定檔
- **相容性**：支援堅果雲、Nextcloud、Synology 等標準 WebDAV 服務（建議使用應用程式密碼）
...
- **文件範本**：範本存放於使用者目錄 `~/Documents/yizimarkdown/templates/`，放入 `.md` 檔案即可使用（也可在 設定 → 範本 中新建/編輯，新建後選單即時更新）；內建範本首次啟動自動同步到該目錄，升級/重裝不會覆蓋你的範本
- **快捷鍵系統**：視覺化快捷鍵配置面板，支援 30 個 action 的自訂綁定、按鍵錄製、衝突檢測和恢復預設
- **設定面板**：通用、外觀、編輯器、即時模式、AI、雲端、插件、快捷鍵、範本、關於等多個標籤頁，設定即時預覽
