...
- **내보내기**：HTML / Markdown / 순수 텍스트 / Word(DOCX) / PDF 다섯 가지 형식; DOCX는 Word 네이티브 시맨틱(목록/표/코드 블록/인용/작업 목록)을 사용하며, 표 스타일은 미리보기와 일치합니다. 이미지는 로컬 경로, 네트워크 이미지, HTML `<img>` 태그를 지원합니다(`width` 속성으로 너비 조정 가능). PDF는 미리보기와 동일한 재현도로 자동 페이지 나누기됩니다
- **.md 파일 연결**：설정에서 원클릭으로 시스템 기본 Markdown 편집기로 설정, .md 더블클릭으로 바로 열기(Windows 레지스트리 / macOS LaunchServices)

### 클라우드 저장소(WebDAV)

- **클라우드 파일 브라우저**: 사이드바의 "클라우드" 탭에서 WebDAV 서버 트리를 탐색하고 문서 열기, 폴더 만들기, 이름 바꾸기, 삭제를 할 수 있습니다
- **클라우드에 저장**: 로컬 문서를 한 번에 업로드(상대 경로로 참조한 이미지도 디렉터리 구조를 유지한 채 함께 업로드). 클라우드 문서라면 `Ctrl+S`로 서버에 바로 기록합니다
- **충돌 보호**: 저장 전에 서버 ETag를 비교하여 다른 기기에서 변경되었다면 덮어쓰기 또는 다시 불러오기를 선택하는 대화상자를 띄웁니다(무단 덮어쓰기 없음)
- **인증 정보 보안**: 사용자 이름과 비밀번호는 운영체제 자격 증명 저장소(Windows 자격 증명 관리자 / macOS 키체인)에 저장되며 설정 파일에 기록되지 않습니다
- **호환성**: Nutstore, Nextcloud, Synology 등 표준 WebDAV 서비스를 지원합니다(앱 전용 비밀번호 사용 권장)

### 외관 사용자 정의

- **15개 내장 테마**：학술 블루, 활기 오렌지, 테크 감성, 미니멀, 매거진, 내추럴, 액체 글래스 Prism(기본), 리치 레드, 바이올렛, 사이버펑크, Facebook, 매트릭스, 민트 스무디, 선셋 골드, 레트로 타이핑기, 각 테마마다 밝고 어두운 두 가지 색상 포함
...
- **문서 템플릿**：템플릿은 사용자 디렉토리 `~/Documents/yizimarkdown/templates/`에 저장되며, `.md` 파일을 넣기만 하면 사용할 수 있습니다(설정 메뉴에서 새로 만들기/편집도 가능하며, 생성 후 메뉴가 즉시 업데이트됩니다); 내장 템플릿은 첫 실행 시 이 디렉토리로 자동 동기화되며, 업그레이드/재설치해도 사용자의 템플릿은 그대로 유지됩니다
- **단축키 시스템**：시각적 단축키 설정 패널, 30개 액션의 사용자 정의 바인딩, 키 녹음, 충돌 감지 및 기본값 복원 지원
- **설정 패널**：일반, 외관, 편집기, 실시간 모드, AI, 클라우드, 플러그인, 단축키, 템플릿, 정보 등 여러 탭으로 구성, 설정 즉시 미리보기
...
│   ├── magazine.css        # 매거진
│   ├── nature.css          # 내추럴
│   ├── liquidglass-prism.css # 액체 글래스 Prism(기본)
│   ├── lychee.css          # 리치 레드
│   ├── violet.css          # 바이올렛
│   ├── cyberpunk.css       # 사이버펑크
...
└── templates/              # 내장 템플릿(첫 실행 시 사용자 디렉토리로 동기화)
    └── default.md          # 기본 템플릿
```
