...
- **元信息属性面板**：实时模式把 YAML front matter 渲染成 Obsidian 式可编辑属性面板（自动 / 文本 / 数字 / 复选框 / 日期 / 日期和时间 / 列表 7 种类型，类型按属性名全局记忆）；源码模式原样、预览模式隐藏
...
- **Tab 标签栏**：顶部管理多个打开的文件，切换、关闭、新建，**支持拖动排序**
...
- **导出**：HTML / Markdown / 纯文本 / Word（DOCX）/ PDF 五种格式；DOCX 使用 Word 原生语义（列表/表格/代码块/引用/任务列表），表格样式对齐预览，图片支持本地路径、网络图片与 HTML `<img>` 标签（可用 `width` 属性控制宽度）；PDF 与预览还原度一致并自动分页；HTML 附带**左侧悬浮大纲卡片**，点击条目直达对应章节（PDF 不含侧栏）
- **.md 文件关联**：设置中一键设为系统默认 Markdown 编辑器，双击 .md 直接打开（Windows 注册表 / macOS LaunchServices）

### 云端存储（WebDAV）

- **云端文件浏览器**：侧边栏「云端」标签页浏览 WebDAV 服务器目录树，支持打开文档、新建文件夹、重命名、删除；深层目录面包屑折成「…」可下拉跳转，点文件夹图标可逐层懒加载展开
- **加载预设**：设置 → 云端存储，地址框聚焦即出现「加载预设」，一键填入坚果云地址
...
├── themes/                 # 主题 CSS 文件
│   ├── liquidglass-prism.css # 液态玻璃 Prism（默认）
│   ├── academic.css        # 学术蓝
│   ├── vibrant.css         # 活力橙
│   ├── tech.css            # 科技感
│   ├── minimal.css         # 极简风
...
└── templates/              # 内置文档模板（首次启动同步到用户目录）
    └── default.md          # 默认模板
```

> 提示：AI 技能与文档模板的真实存储位置都在用户目录 —— `~/Documents/yizimarkdown/skills/` 与 `~/Documents/yizimarkdown/templates/`（程序目录下的同名文件夹是内置资源，首次启动会同步过去）。自定义内容请放用户目录，升级/重装不会被覆盖。
