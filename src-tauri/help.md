...
- 云端文档的 Ctrl+S 与自动保存都会写回服务器（见下方「云端存储」）
...
- **HTML**：保留样式，导出为自包含文档；附带**左侧悬浮大纲卡片**，点击条目直达对应章节（PDF 不含侧栏）
- **Markdown**（纯文本）
- **纯文本**（去除所有格式）
...
切换视图时自动定位到当前编辑位置；**停在顶部时不做补偿**，避免刚进模式就被被动滚走。

### 元信息属性面板（实时模式）

文档开头的 YAML front matter（`---` 包裹）在三种模式下各有规范显示：

- **源码模式**：原样零装饰（所见即文件）
- **实时模式**：渲染为 **Obsidian 式属性面板** —— 每行 = [类型图标] [属性名] [值]，可直接编辑；点类型图标可切换 7 种属性类型（自动 / 文本 / 数字 / 复选框 / 日期 / 日期和时间 / 列表），类型按属性名全局记忆
- **预览模式**：隐藏（阅读模式主流做法）

表头「编辑源码」可把光标落到第一个属性行直接编辑原始 YAML。
...
- 显式指令：`<!-- layout: xxx -->` 强制指定版式（非法值忽略并回落自动推断）、`<!-- align: left|center|right -->` 整页对齐（HTML 注释，渲染不可见）
...
内容页 / 列表页 / 路线图页的**顶层列表项**可逐条出现：按 `→` 先显示下一条，全部显示后再按才翻页；`←` 先回退一条，退完再翻页（翻回的页面列表全显）。讲哪条，观众看哪条。

**默认关闭**（列表整页显示，避免目录等内容被迫逐条点击）。开启方式：

- 播放中：点击 HUD 底部的列表图标（即时生效，仅本次播放）
- 整副开启：front matter 写 `slideshow-fragments: on`
- 单页开启 / 关闭：页内写 `<!-- fragments: on -->` 或 `<!-- fragments: off -->`

- 仅上述三种版式生效；封面/章节/目录/表格/图表等版式整页显示
- 嵌套子列表随父项一起出现，无需单独标记

### 切换动画

播放中点击 HUD 底部的 ✨ 图标可切换翻页动画（仅本次播放生效）：
...
|------|------|
| 水平滑动 | 新页从右滑入 + 淡入（默认） |
| 淡入淡出 | 纯透明度溶解，类似 Keynote |
| 轻微缩放 | 轻微缩放进出 |
| 无动画 | 直接切换 |

**强烈型**（棋盘格子/百变立方/六角蜂巢/爆裂卡片/纵深穿梭/百叶窗格 同属"瓷砖转场"：整页连同背景被切成单元，每个单元自带前一页与后一页的内容，因此可以各自独立翻转、各自不同节奏）：

| 动画 | 效果 |
|------|------|
| 高斯溶解 | 水渍从多点渗开、互相吞并，有机边缘吞没整页（新页从模糊中清晰起来） |
| 百叶窗格 | 整页切成竖条，每条绕自身中轴翻面露出下一页 |
| 棋盘格子 | 整页拆成方格，随机次序 3D 翻面露出下一页，随后融合回落 |
| 百变立方 | 每格绕自身竖轴转 180°，中途露出侧面，像一个个小立方体在转 |
| 六角蜂巢 | 整页切成蜂巢状六边形，各自随机次序翻面（带卡片外阴影） |
| 爆裂卡片 | 旧页碎片带随机方向旋转飞散，新页碎片从反方向飞入归位 |
| 纵深穿梭 | 每格有不同纵深，翻面时前后穿梭，产生空间视差 |
| 卡片推换 | 当前页缩小成卡片向左移出，新页卡片从右进入后放大落定（后退镜像） |
...
把文档直接读写到 WebDAV 服务器（坚果云、Nextcloud、群晖等），在多台设备之间共用同一份笔记。

### 配置服务器

打开 设置 > 云端存储：

1. **服务器地址**：WebDAV 根地址，需包含完整路径
   - 坚果云：`https://dav.jianguoyun.com/dav/`
   - Nextcloud：`https://你的域名/remote.php/dav/files/用户名/`
   - 也可在地址框**聚焦时点「加载预设」**，一键填入坚果云地址
2. **用户名 / 密码**：填好后点「保存凭据」
   - 多数服务需要**应用专用密码**而非登录密码：坚果云在「账户信息 → 安全选项 → 添加应用」生成，Nextcloud 在「设置 → 安全 → 应用密码」生成
3. 点「测试连接」确认可用

> 用户名与密码保存在系统凭据库（Windows 凭据管理器 / macOS 钥匙串），不写入任何配置文件，也不进入浏览器存储。

### 浏览云端文件

侧边栏切换到「云端」标签页：

- 点击文件夹进入，顶部面包屑可回上级；层级较深时上级折成「…」，点「…」可下拉跳转任意隐藏层级
- 点**文件夹图标**可就地逐层展开子目录（懒加载），带层级引导线
- 点击文档直接打开，正文与文档中相对引用的图片会一并下载到本地缓存
- 悬停行尾的 `⋯` 或右键，可**新建文件夹 / 重命名 / 删除**（删除不可撤销，删文件夹会一并删除其中的内容）
- 右上角 ⟳ 手动刷新；保存到云端后若正浏览该目录，列表会自动刷新

### 保存到云端

- **把本地文档上传到云端**：点工具栏「保存到云端」，确认云端路径后上传。文档中相对引用的图片会一并上传并保持目录结构
- **保存已打开的云端文档**：直接 Ctrl+S，写回它原本的远程位置
- **另存到其他云端位置**：同样是「保存到云端」，把路径改成新的即可

保存云端文档时是「先写本地缓存、再上传服务器」，**两步都成功才会标记为已保存**；任一步失败都会保持未保存状态并提示。

### 冲突保护

打开云端文档时会记录服务器的 ETag。若保存前发现该文件已被其他设备修改，会弹窗让你选择：

- **覆盖服务器版本**：用本地内容覆盖服务器
- **放弃本地修改并重新加载**：丢弃本地编辑，重新拉取服务器版本

绝不会静默覆盖。自动保存遇到冲突时会暂停并提示，按 Ctrl+S 手动处理。

### 注意事项

- 需要服务端支持 WebDAV。若登录失败并提示需要 Digest 认证，请改用应用专用密码
- 文档中**绝对路径**的图片（如 `D:\pics\a.png`）不会被上传 —— 只有相对路径的图片才随文档同步
- 云端文档会在本地保留一份缓存（`~/Documents/yizimarkdown/webdav/`），用于图片渲染与 DOCX/PDF 导出；删除它不影响云端数据，下次打开会重新下载
...
>
> v0.3.0 起云端文档的本地缓存位于 `~/Documents/yizimarkdown/webdav/`，仅为派生物，可随时删除。
...
6. **WebDAV 凭据（v0.3.0 起）**：与 API 密钥同理，保存在系统凭据库（Windows 凭据管理器 / macOS 钥匙串），**不写入任何配置文件**；随时可在「设置 → 云端存储」清除
7. **云端文档缓存（v0.3.0 起）**：存放在 `~/Documents/yizimarkdown/webdav/`，仅用于图片渲染与导出。它是**派生物**，删除不影响云端数据，下次打开会自动重新下载

**Q: 云端文档改了会丢吗？多台设备同时改怎么办？**
A: 打开云端文档时会记录服务器的 ETag；保存前若发现文件已被其他设备修改，会弹窗让你选择「覆盖服务器版本」或「放弃本地修改并重新加载」，**不会静默覆盖**。自动保存遇到冲突会暂停并提示，按 Ctrl+S 手动处理。

**Q: 为什么云端文档里的图片没有一起上传？**
A: 只有**相对路径**的图片（如 `images/a.png`、`../assets/x.png`）会随文档同步，并保持目录结构。绝对路径（`D:\pics\a.png`、`/home/me/a.png`）、`file://`、网络图片（`http(s)://`）与 `data:` 都不会上传——这是为了避免把你本机其他位置的文件传到服务器。

**Q: 测试连接提示「服务器要求 Digest 认证」怎么办？**
A: 本程序只支持 Basic 认证。请改用**应用专用密码**（坚果云在「账户信息 → 安全选项 → 添加应用」生成，Nextcloud 在「设置 → 安全 → 应用密码」生成），而不是登录密码。

**Q: 登录失败，提示用户名或密码错误？**
A: 多数 WebDAV 服务（坚果云、开启两步验证的 Nextcloud）不接受登录密码，必须使用**应用专用密码**。另外请确认服务器地址包含完整路径，例如坚果云是 `https://dav.jianguoyun.com/dav/`（注意结尾的 `/dav/`）。
