...
- **整页结构推断**：分析整页的标题层级、块类型与组合方式，自动选择最合适的版式
- **14 种自动版式**：封面、章节过渡页、结尾页、目录页、内容页、列表页、数据表页、路线图页、图文页、图片页、金句页、代码页、图表页、公式页
- **金句页重设计**：去掉左侧竖线；内容上下居中；大引号改为对角装饰（上引号挂文案左上角、下引号挂右下角，6em、35% 透明度）
- **封面 meta**：front matter 提供 `author`/`date` 时，封面页底部自动显示「作者 · 日期」
- **显式指令**：`<!-- layout: xxx -->` 强制版式、`<!-- align: left|center|right -->` 整页对齐，覆盖自动推断
- **装饰层**：左下角章节名 + 页码页脚，底部主题色进度条随翻页增长
- **鼠标滚轮翻页**：内容可滚动时先滚动内容，滚到上/下边界再翻页；翻页后短暂锁定防惯性连翻

**应用图标更新 + 演示模板图片资源修复**

- **主程序图标更新**：`icon.png` 替换为最新版，重新生成 `icon.ico` / `icon.icns`；文件关联图标 `md-icon.ico` 保持不变
- **演示模板更新**：`Slide-Template.md` 置换为 `slideshow-test.md` 内容（版式推断测试示例）
- **模板图片相对路径**：示例图片迁入 `templates/images/`，模板相对路径引用，可移植
- **新增 `src/lib/localImages.ts`**：预览/演示的本地图片统一解析，支持相对与绝对路径
- **修复打包丢图**：资源映射改为目录形式，递归打包 `templates/images/` 子目录

## v0.1.9

**演示模式（幻灯片）**

- 将任意 Markdown 文档按 `---` 分页渲染成应用内全屏幻灯片，模拟 PPT 播放
- **纯 Markdown 驱动**：不需要任何额外格式，引擎根据内容结构自动选择版式
...
- **模板功能完善**：「从模板新建」入口、修复默认模板 bug、模板管理
